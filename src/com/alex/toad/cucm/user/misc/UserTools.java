...
public class UserTools
	 ...
	 * !!!! Modify with care !!!!!
	 */
	public static ArrayList<ItemToInject> setUserItemList(AgentData ad, actionType action, UserCreationProfile ucp) throws Exception
		{
		ArrayList<ItemToInject> itemList = new ArrayList<ItemToInject>();
		
		Variables.getLogger().debug("Processing user : "+ad.getFirstName()+" "+ad.getLastName());
		...
		try
			...
			if((myUser.getAssociatedItems().size() != 0) && (!foundMIU))itemList.add(myUser);
			}
